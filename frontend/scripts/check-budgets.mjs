...
const root = ".next";
const budgets = { homepage: 95_000, catalog: 115_000, category: 115_000, pdp: 100_000 };
...
let failed = false;
for (const [route, file] of Object.entries(manifests)) { const candidates = manifestFiles.filter((candidate) => candidate.includes(route === "homepage" ? "/[locale]/page" : route === "catalog" ? "/shop/" : route === "category" ? "/category/" : "/product/")); const selected = (candidates[0] || file).replace(`${root}/`, ""); if (!fs.existsSync(`${root}/${selected}`)) { console.error(`Missing build manifest for ${route}. Run next build first.`); failed = true; continue; } const bytes = routeBytes(selected); console.log(`${route}: ${(bytes / 1024).toFixed(1)} KiB JS (budget ${(budgets[route] / 1024).toFixed(1)} KiB)`); if (bytes > budgets[route]) failed = true; }
console.log(`global CSS: ${(css / 1024).toFixed(1)} KiB (soft budget 180.0 KiB)`);
if (css > 350_000) failed = true;
