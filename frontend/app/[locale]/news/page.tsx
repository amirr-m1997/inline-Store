...
export default async function NewsPage({ params, searchParams }: PageProps) {
  ...
  return <main className="editorial-center site-container">
    ...
    {unique.length > 0 ? <section className="editorial-latest editorial-news-list" aria-labelledby="news-list-title"><header><div><span>{english ? "Published updates" : "به‌روزرسانی‌های منتشرشده"}</span><h2 id="news-list-title">{type ? (editorialTypes.find((item) => item.value === type)?.[english ? "en" : "fa"] || "") : (english ? "Latest news and events" : "جدیدترین اخبار و رویدادها")}</h2></div><span className="editorial-result-count">{formatNumber(unique.length)}</span></header><div className="editorial-card-grid">{unique.map((article) => <EditorialCard key={article.id} article={article} locale={locale} />)}</div></section> : <div className="editorial-empty"><h2>{english ? "No published news is available yet." : "هنوز خبر منتشرشده‌ای در دسترس نیست."}</h2><p>{english ? "New company updates will appear here when published." : "خبرهای جدید شرکت پس از انتشار در این بخش نمایش داده می‌شوند."}</p></div>}
