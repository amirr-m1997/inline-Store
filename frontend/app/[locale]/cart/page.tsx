...
const money = (value: string | null | undefined, locale = "fa") => {
  ...
  return `${numeric.toLocaleString(locale === "en" ? "en-US" : "fa-IR")} ${locale === "en" ? "IRR" : "ریال"}`;
};
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_380px]">
      <section className="card">
        ...
        {cart.items.map((item) => { const productImage = item.product.images?.find((image) => image.is_primary) ?? item.product.images?.[0]; return <article className="cart-item mt-4 grid gap-3 border-b pb-4 sm:grid-cols-[1fr_auto] sm:items-center" key={item.id}>
          ...
          <div className="cart-item-controls" aria-label={`کنترل‌های ${item.product.name}`} aria-busy={updatingItem === item.id}><button type="button" className="h-9 w-9 rounded border" disabled={updatingItem === item.id} onClick={() => void changeQuantity(item, item.quantity - 1)} aria-label={`کاهش تعداد ${item.product.name}`}>−</button><b className="min-w-8 text-center" aria-live="polite">{updatingItem === item.id ? "…" : item.quantity.toLocaleString("fa-IR")}</b><button type="button" className="h-9 w-9 rounded border" disabled={updatingItem === item.id || (item.product.available_quantity !== null && item.quantity >= item.product.available_quantity)} onClick={() => void changeQuantity(item, item.quantity + 1)} aria-label={`افزایش تعداد ${item.product.name}`}>+</button><button type="button" className="me-2 rounded border border-red-200 px-3 py-2 text-xs text-red-700" disabled={updatingItem === item.id} onClick={() => void changeQuantity(item, 0)} aria-label={`حذف ${item.product.name} از سبد`}>حذف</button></div>
        </article>; })}
