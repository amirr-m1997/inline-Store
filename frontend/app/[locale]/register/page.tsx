...
import { authChanged, mergeGuestCart, parseError } from "../../../services/auth-client";
...
export default function RegisterPage() {
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => { event.preventDefault(); setLoading(true); setError(""); const data = Object.fromEntries(new FormData(event.currentTarget)); const response = await fetch("/api/v1/auth/register/", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(data) }); setLoading(false); if (!response.ok) return setError(await parseError(response)); await finish(); };
  return <main className="auth-page"><section className="auth-panel auth-panel-wide"><header><span>عضویت مشتریان</span><h1>ایجاد حساب کاربری</h1><p>ثبت حداقل یکی از ایمیل یا شماره موبایل کافی است.</p></header><form className="auth-form auth-grid" onSubmit={submit}><label>نام *<input name="first_name" autoComplete="given-name" required /></label><label>نام خانوادگی *<input name="last_name" autoComplete="family-name" required /></label><label>ایمیل<input name="email" type="email" dir="ltr" autoComplete="email" /></label><label>شماره موبایل<input name="phone_number" dir="ltr" inputMode="tel" autoComplete="tel" placeholder="09123456789" /></label><label>نام شرکت<input name="company_name" /></label><span />
    <label>رمز عبور *<div className="password-field"><input name="password" type={visible ? "text" : "password"} dir="ltr" value={password} onChange={(e) => setPassword(e.target.value)} autoComplete="new-password" required /><button type="button" onClick={() => setVisible((value) => !value)}>{visible ? "پنهان" : "نمایش"}</button></div><small className={`password-strength strength-${strength}`}>قدرت رمز: {strength < 2 ? "ضعیف" : strength < 4 ? "متوسط" : "قوی"}</small></label><label>تکرار رمز عبور *<input name="password_confirm" type={visible ? "text" : "password"} dir="ltr" autoComplete="new-password" required /></label>
