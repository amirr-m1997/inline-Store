...
import { getSupportRequests, submitSupport } from "../../lib/api/content";

type RecordItem = { reference: string; request_type: string; status: string; subject: string; submitted_at: string; product?: number | null; order?: number | null };
...
export function SupportRequestForm({ english = false }: { english?: boolean }) {
  ...
  const [history, setHistory] = useState<RecordItem[] | null>(null);
  ...
  useEffect(() => { getSupportRequests<RecordItem>().then(setHistory).catch(() => setHistory([])); }, []);
  ...
  return <div className="support-request-stack">
    ...
    {history?.length ? <section><h2>{t("درخواست‌های من", "My requests")}</h2>{history.map(item => <article className="content-card" key={item.reference}><code dir="ltr">{item.reference}</code><h3>{item.subject}</h3><p>{item.status}</p></article>)}</section> : null}
