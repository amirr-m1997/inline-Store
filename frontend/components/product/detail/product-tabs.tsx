...
export function ProductTabs({ description, specifications }: { description: string; specifications: ReactNode }) {
  const [activeTab, setActiveTab] = useState<"specs" | "description">(description ? "description" : "specs");
  const tabs = [...(description ? [{ id: "description", label: "معرفی محصول", enabled: true }] : []), { id: "specs", label: "مشخصات فنی", enabled: true }];
  ...
  </div>{activeTab === "specs" ? <div id="product-panel-specs" role="tabpanel" aria-labelledby="product-tab-specs" tabIndex={0}>{specifications}</div> : <div id="product-panel-description" role="tabpanel" aria-labelledby="product-tab-description" tabIndex={0} className="product-description-body" dangerouslySetInnerHTML={{ __html: description }} />}</section>;
}
