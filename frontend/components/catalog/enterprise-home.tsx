...
function ProductSection({ locale, title, kicker, products, href, empty, tone = "supporting" }: { locale: string; title: string; kicker: string; products: CatalogProduct[]; href?: string; empty: string; tone?: "featured" | "supporting" }) {
  ...
  return <section className={`enterprise-section home-product-section home-product-section--${tone} site-container`}>
    ...
    {products.length ? <div className="industrial-product-grid">{products.slice(0, 3).map((product) => <ProductCard key={product.id} product={product} />)}</div> : <div className="home-section-empty">{empty}</div>}
  </section>;
}
...
export function EnterpriseHome({ locale, company, hero, advantages, categories, featured, newest, discounted, brands, industries, capabilities, editorial }: HomepageInitialData) {
  ...
  return <main className="enterprise-home">
    ...
    <section id="categories" className="enterprise-section site-container" aria-labelledby="home-categories-title"><header className="home-section-heading"><div><div className="section-kicker">{english ? "Quick access" : "دسترسی سریع"}</div><h2 id="home-categories-title">{english ? "Main product categories" : "دسته‌بندی‌های اصلی"}</h2></div><Link href={`/${locale}/categories`}>{english ? "View all" : "مشاهده همه"} <span aria-hidden="true">←</span></Link></header>{categories.length ? <div className="root-cards">{categories.slice(0, 4).map((category) => <Link key={category.id} href={getCategoryUrl(category, locale)} aria-label={`${english ? "View category" : "مشاهده دسته"} ${category.name_fa}`}><span>{english ? "Main category" : "دسته اصلی"}</span><b>{category.name_fa}</b><i>{english ? "Explore subcategories" : "مشاهده زیرگروه‌ها"} <span aria-hidden="true">←</span></i></Link>)}</div> : <div className="home-section-empty">{english ? "Product categories are not available." : "دسته‌بندی‌های محصولات در دسترس نیست."}</div>}</section>
