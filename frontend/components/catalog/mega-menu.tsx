...
import { getCategoryChildren, getCategoryRoots } from "../../lib/api/products";
...
function Branch({ category, locale }: { category: NavCategory; locale: string }) {
  ...
  const load = () => {
    ...
    getCategoryChildren<NavCategory[]>(category.id).then((data) => { setChildren(data); setLoaded(true); }).catch(() => setLoaded(true));
  };
...
export function MegaMenu() {
  ...
  const [children, setChildren] = useState<NavCategory[]>([]);
  useEffect(() => { getCategoryRoots<NavCategory[]>().then(setRoots).catch(() => setRoots([])); }, []);
  const open = (root: NavCategory) => {
    setActive(root);
    getCategoryChildren<NavCategory[]>(root.id).then(setChildren).catch(() => setChildren([]));
  };
