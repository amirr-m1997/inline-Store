...
function TreeNode({ category, locale }: { category: Category; locale: string }) {
  ...
  const toggle = () => { setOpen((value) => !value); if (!children.length && category.has_children) getCategoryChildren<Category[]>(category.id).then(setChildren).catch(() => setChildren([])); };
...
export function CategoryDirectory() {
  ...
  useEffect(() => { getCategoryRoots<Category[]>().then(setRoots).catch(() => setRoots([])); }, []);
