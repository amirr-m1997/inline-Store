...
export type ProductResource = { id: number; title: string; type: "datasheet" | "manual" | "cad" | "certificate" | "other"; file_url: string; file_name: string; display_name: string; mime_type: string; size: number; revision: string; language: string; product: { id: number; name: string; code: string; slug: string } };
...
export async function getProductDocumentsServer(parameters: Record<string, string | undefined> = {}): Promise<ProductResource[]> { const query = new URLSearchParams(); Object.entries(parameters).forEach(([key, value]) => { if (value) query.set(key, value); }); return getServerProducts<ProductResource[]>(`/api/v1/catalog/documents/${query.toString() ? `?${query}` : ""}`); }
